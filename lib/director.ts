...
export async function toCard(item: Item): Promise<Card> {
  const base: Card = {
    id: item.key,
    kind: item.kind,
    title: item.title,
...
export async function pick(
  ...
  const shown = new Set([...(opts.exclude ?? []), ...(await shownKeys())]);
  ...
  if (await factByTag("spicy")) want.add("spicy");
  if (await factByTag("light")) want.add("light");
  out = [...out].sort(
    (a, b) =>
...
async function mealBeats(kind: "lunch" | "dinner"): Promise<Beat[]> {
  const isLunch = kind === "lunch";
  const slot = isLunch ? "noon" : "evening";
  // 主动推送里人格是价值:两个人一来一回,但只出一张清单卡,别让用户看不过来
  const { list } = await filterDishes("g-eat", {
    exclude: await shownRecoKeys("g-eat"),
  });
...
async function weekendBeats(): Promise<Beat[]> {
  const ex = await shownRecoKeys("g-weekend");
  const { list } = await filterOutings("g-weekend", { exclude: ex });
...
async function morningBeats(): Promise<Beat[]> {
  ...
  const card: Card = {
    ...
    reason: sleepFact?.text ?? undefined,
  };
  return [
...
export async function eatEntryBeats(): Promise<Beat[]> {
  const slot = slotOf();
  ...
  for (const c of cats) {
    const { list, relaxed } = await filterDishes("g-eat", {
      cat: c,
...
export async function weekendEntryBeats(): Promise<Beat[]> {
  const slot = slotOf();
  const ex = await shownRecoKeys("g-weekend");
  const picked: Outing[] = [];
  let relaxed = false;
  ...
  for (const who of ["jingshen", "majiaxian", "lvyou"]) {
    const { list, relaxed: r } = await filterOutings("g-weekend", {
...
export async function entryBeats(conversationId: string): Promise<Beat[]> {
 ...
 * 同品类里凑不够就如实说,不跨品类硬凑。
 */
export async function similarBeats(conversationId: string, key: string): Promise<Beat[]> {
  const ex = await shownRecoKeys(conversationId);
  ...
  const { list, relaxed } = await gather();
...
export async function triggerBeats(kind: TriggerKind) {
  switch (kind) {
    case "lunch":
      return { conversationId: "g-eat", beats: await mealBeats("lunch") };
    case "dinner":
      return { conversationId: "g-eat", beats: await mealBeats("dinner") };
    case "weekend":
      return { conversationId: "g-weekend", beats: await weekendBeats() };
    case "morning":
      return { conversationId: "c-mimi", beats: await morningBeats() };
    case "diary": {
...
export async function replaceBeats(
  agentId: string,
  kind: Item["kind"],
  reason: string,
): Promise<Beat[]> {
  const next = await pick(kind, { drop: [reason], n: 1 });
  await markShown(
    next.map((i) => i.key),
    "g-eat",
  );
  ...
  await logEvent("reject", `拒了一个推荐,原因是${reason}`, reason === "贵" ? "budget" : undefined);
  const cards = await Promise.all(next.map(toCard));
  return [
    { speaker: agentId, text: line[reason] ?? "行,换一个", gapMs: 420 },
    { speaker: agentId, text: "这个呢", cards },
  ];
}
...
export async function respondBeats(
  ...
  await logEvent("chat", `跟${members.map((m) => AGENTS[m].name).join("、")}说了:${userText}`);
  ...
  if (llmReady()) {
    ...
    const out = await chatJSON<{ beats: (Beat & { cardKey?: string })[] }>(
      ...
      [
        ...
        `用户画像与记忆:${facts.slice(0, 10).map((f) => f.text).join(";") || "还很少"}`,
    ...
    if (out?.beats?.length) {
      const results: Beat[] = [];
      for (const b of out.beats.filter((x) => responders.includes(x.speaker)).slice(0, 4)) {
        ...
        results.push({
          ...
          mention: b.mention,
          chips: b.chips,
          cards: item ? [await toCard(item)] : undefined,
        } satisfies Beat);
      }
      return results;
    }
 ...
 */
async function ruleReply(members: string[], text: string): Promise<Beat[]> {
  ...
  const dishCard = async (
    opts: { cat?: string; serve?: any } = {},
    who: string,
    title: string,
  ) => {
    const { list } = await filterDishes("g-eat", {
  ...
  if (has("贵", "便宜", "省", "穷")) {
    ...
    return [
      ...
      { speaker: who, cards: [await dishCard({}, who, "便宜好吃的三个")] },
    ];
  }
  // 口味
  if (has("辣", "清淡", "减脂", "油")) {
    ...
    return [
      {
        speaker: who,
        text: light ? "行,把重口的放一边 🥗" : "要辣的是吧,那我不客气了 🔥",
        gapMs: 440,
      },
      {
        speaker: who,
        cards: [await dishCard({ cat: light ? "轻食" : "川湘" }, who, light ? "清淡的三个" : "够辣的三个")],
      },
    ];
  }
  // 想出门
  if (has("出门", "到店", "去店", "堂食")) {
    ...
    return [
      ...
      { speaker: who, cards: [await dishCard({ serve: "dine" }, who, "走过去就能吃的三家")] },
    ];
  }
  // 懒得动
  if (has("累", "烦", "不想", "懒")) {
    ...
    return [
      ...
      { speaker: who, cards: [await dishCard({ serve: "deliver" }, who, "送到门口的三个")] },
    ];
  }
  // 周末相关
  if (has("周末", "去哪", "玩", "展", "演出", "运动", "爬山")) {
    const who = members.includes("jingshen") ? "jingshen" : members[0];
    const { list } = await filterOutings("g-weekend", {
      exclude: await shownRecoKeys("g-weekend"),
    });
