  ...
  withTenant(async () => {
    ...
    if (!id) {
      const rows = await all(
        "SELECT id,day,created_at FROM plans WHERE tenant=? ORDER BY created_at DESC",
        [tenantId()],
      );
  ...
  withTenant(async () => {
    ...
    if (confirm) {
      ...
      for (const i of p.items) {
        await run(
          "INSERT INTO todos (tenant,id,title,due,source,done,created_at) VALUES (?,?,?,?, 'plan',0,?)",
          [t, uid("t-"), i.title, `${p.day} ${i.start}`, Date.now()],
        );
      }
      await run("UPDATE plans SET confirmed=1 WHERE tenant=? AND id=?", [t, id]);
