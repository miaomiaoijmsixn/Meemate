...
export const POST = (req: Request) =>
  ...
  if (dish || outing) {
    if (b.action === "replace") {
      const beats = await similarBeats(b.conversationId, b.cardId);
      if (beats.length) await enqueue(b.conversationId, beats, Date.now() + 320);
    ...
    if (b.action === "wish") {
      ...
      if (!exist) {
        ...
        await logEvent("wish", `把${title}加进了清单`);
      }
      await enqueue(
        b.conversationId,
  ...
  if (b.action === "replace") {
    ...
    const beats = await replaceBeats(b.agentId, kind, b.reason ?? "换一个");
    await enqueue(b.conversationId, beats, Date.now() + 300);
    return Response.json({ ok: true });
  }
  ...
  if (b.action === "wish") {
    ...
    if (!exist) {
      ...
      await logEvent("wish", `把《${item.title}》加进了愿望清单`);
    }
    await enqueue(b.conversationId, wishAckBeats(b.agentId, item.title), Date.now() + 300);
    return Response.json({ ok: true });
  }
